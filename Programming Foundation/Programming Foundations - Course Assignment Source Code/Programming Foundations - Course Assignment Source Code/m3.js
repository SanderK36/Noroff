...
function addTwoNumbers() {
    ...
    if(!isNaN(num1) && !isNaN(num2)) {       
        global = parseInt(num1) + parseInt(num2); 
        alert("The final score is: " + (parseInt(num1) + parseInt(num2)));   
    }
    else {
        alert("Wrong data");
    }
...
function extractMiddleString() {
    ...
    const startIndex = Math.round(text.length / 2);
...
function stringReplace() {
    ...
    global = text.replace(/a/ig, '@').replace(/e/ig, '3').replace(/o/ig, '0');
    alert(global);  
}
...
function spliceExample() {
    ...
    if(index >= 0) {
        ...
        global = spliceText; alert(global);
    }
}
